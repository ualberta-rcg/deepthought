...
- Pipelines are fully separate and basic (no service containers, ever): build-cli.yml triggers on deepthought-cli/** only (vet + test + race + ONE static binary → artifact + rolling edge release for curl installs); build-server.yml triggers on deepthought-server/** only (docker build from the self-contained server context + push; deployment does NO testing).
- Files: new deepthought-server/{go.mod,go.sum,Dockerfile,README.md,graph/,k8s/}; moved+adapted server/{http,auth,settings,db,runtime}.go, web/, store/; deleted deepthought-cli/{internal/server,cmd/deepthought-server,Dockerfile}; both workflows; root README/CLAUDE.md; this entry.
- Verification: go vet + go test green in BOTH modules locally; the wire contract compiles identically; this push runs both separated pipelines — iterate until green.
...
- Verification: the workflows' structure mirrors the already-green build-server.yml lanes; this push triggers both and is their first live run.

## 2026-09-21 · deepthought-cli — server-side MySQL: multi-user store, settings sync, chat sync
- internal/history/mysql.go + mysql_records.go: MySQLStore, the multi-user server port of SQLiteStore behind the same ChatStore interface. Same polymorphic interactions schema + user_id scoping on every table and query; the filesystem blob spill becomes a sha256-keyed `bodies` table; a real connection pool replaces SetMaxOpenConns(1) with per-save transactions; the unused SQLite tables (links, summaries, body_versions) are not ported; UTC RFC3339Nano VARCHAR timestamps keep the lexicographic ordering. UserStore adds users + user_settings with revision-based optimistic concurrency (409 on conflict). Driver: github.com/go-sql-driver/mysql (pure Go, static image preserved).
- Server (internal/server/db.go + http.go): opens the pool from $DEEPTHOUGHT_MYSQL_DSN (schema ensured at boot; nil DSN → DB endpoints 503 while health/UI/auth keep working); login upserts the user row; new session-authed, user-scoped endpoints: GET/PUT /api/v1/user/settings (If-Match-style revision; 409 on conflict), GET /api/v1/chats (ChatSummary list — replaces the 501), GET/PUT /api/v1/chats/{id} (full collective graph fetch/upsert; app-generated IDs make the coarse per-turn push idempotent).
- CLI: login now syncs the roving settings — server empty → push local (first login seeds); differs → pull down with a key-level change notice ("pulled 3 setting change(s): ~effort, +roles, …"); local edits push back via the new Settings.OnSave hook; each completed turn pushes the collective to the server on the busy→idle edge (async; failures surface as a chat notice, never block). v1 sync limits, deliberately: settings resolution at login only (no continuous merge), Continue-screen server listing and historical chat backfill are follow-ups.
- CI: build-server.yml gains a "Store tests (MySQL)" job with a mysql:8.4 service container running the env-guarded integration tests (golden SQLite-vs-MySQL graph comparison, tenant isolation incl. records KV, settings revision conflicts); they skip locally without DEEPTHOUGHT_MYSQL_DSN.
- Ops: k8s/mysql.yaml reference manifest (PVC on the nfs-models class per the house pattern — fresh subdirectory, nothing existing touched; mysql:8.4 Deployment pinned to control-planes; imperative secrets for root/app passwords + DSN), k8s/deployment.yaml gains the DSN env. Cluster deployment happens gate-by-gate with explicit approval each step.
- Files: internal/history/{mysql.go,mysql_records.go,mysql_test.go} (new); internal/server/{db.go,http.go}; cmd/deepthought-server/main.go; internal/app/{server_login.go,settings.go,model.go}; .github/workflows/build-server.yml; k8s/{mysql.yaml,deployment.yaml}; go.mod/go.sum; this entry.
- Verification: go build ./... clean; Slurm CPU job (def-rahimk) go vet + full go test + race (MySQL tests skip without a DSN — see CI for the DB-covered runs); CI runs the MySQL integration suite against the service container; cluster end-to-end follows the gated deploy.
